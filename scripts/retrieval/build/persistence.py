...
def delete_reference_document_subtrees(
    connection: sqlite3.Connection,
    *,
    ...
    if section_ids:
        ...
        statement_rows = connection.execute(
            f"SELECT statement_id FROM statements WHERE section_id IN ({section_placeholders})",
            tuple(section_ids),
        ).fetchall()
        statement_ids = [str(row[0]) for row in statement_rows]
        chunk_rows = connection.execute(
            f"SELECT chunk_uid FROM chunks WHERE section_id IN ({section_placeholders})",
            tuple(section_ids),
        ).fetchall()
        chunk_ids = [str(row[0]) for row in chunk_rows]
        if statement_ids:
            stmt_placeholders = ", ".join("?" for _ in statement_ids)
            connection.execute(
                f"DELETE FROM statements_fts WHERE statement_id IN ({stmt_placeholders})",
                tuple(statement_ids),
            )
            connection.execute(
                f"DELETE FROM statements WHERE statement_id IN ({stmt_placeholders})",
                tuple(statement_ids),
            )
        if chunk_ids:
            ...
            connection.execute(
                f"DELETE FROM chunk_fts_rowids WHERE chunk_uid IN ({chunk_placeholders})",
                tuple(chunk_ids),
            )
            connection.execute(
                f"DELETE FROM chunks_fts WHERE chunk_uid IN ({chunk_placeholders})",
                tuple(chunk_ids),
            )
            connection.execute(
...
def upsert_reference_source_rows(
    connection: sqlite3.Connection,
    *,
    snapshot_id: str,
    documents: list[SourceDocument],
    ...
    for document in documents:
        connection.execute(
            ...
            INSERT OR REPLACE INTO source_documents(
                document_id, snapshot_id, chapter_id, rel_path, title, source_sha256,
                source_fetched_at, source_commit_sha, order_index
            ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                document.document_id,
        ...
        connection.execute(
            ...
            INSERT OR REPLACE INTO docs(
                doc_uid, source_path, title, revision, fetched_at, source_sha256, chapter_id, order_index
            ) VALUES(?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                document.document_id,
    ...
    for section in sections:
        connection.execute(
            ...
            INSERT OR REPLACE INTO sections(
                section_id, snapshot_id, document_id, chapter_id, anchor, heading, order_index,
                level, text, source_sha256, source_fetched_at, source_commit_sha
            ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                section.section_id,
    ...
    for statement in statements:
        connection.execute(
            "DELETE FROM statements_fts WHERE statement_id = ?",
            (statement.statement_id,),
        )
        connection.execute(
            """
            INSERT OR REPLACE INTO statements(
                statement_id, section_id, statement_type, text, confidence, sentence_index,
                source_sha256, source_fetched_at, source_commit_sha
            ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                statement.statement_id,
        ...
        connection.execute(
            """
            INSERT OR REPLACE INTO statements_fts(statement_id, section_id, section_heading, statement_text)
            VALUES(?, ?, ?, ?)
            """,
            (
                statement.statement_id,
    ...
    for chunk in chunks:
        ...
        connection.execute(
            ...
            INSERT OR REPLACE INTO chunks(
                chunk_uid, section_id, raw_text, clean_text, char_len, token_len, source_sha256,
                source_fetched_at, source_commit_sha, order_index
            ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
        ...
        connection.execute(
            """
            INSERT OR REPLACE INTO chunks_fts(chunk_uid, section_id, section_heading, chunk_text)
            VALUES(?, ?, ?, ?)
            """,
            (
                chunk.chunk_uid,
    ...
    refresh_chunk_fts_rowids(connection)
    enforce_chunk_fts_mapping(connection, context="reference source row upsert")
    for chunk_span in chunk_spans:
        connection.execute(
            """
            INSERT OR REPLACE INTO chunk_spans(chunk_uid, source_anchor, start_offset, end_offset, span_order)
            VALUES(?, ?, ?, ?, ?)
            """,
            (
                chunk_span.chunk_uid,
...
def refresh_reference_derived_tables(
    connection: sqlite3.Connection,
    *,
    commit_sha: str,
    fetched_at: str,
    ...
    connection.execute(
        """
        INSERT INTO kb_metadata(kb_id, source_name, source_revision, extractor_version, built_at, notes)
        VALUES(?, ?, ?, ?, ?, ?)
        """,
        (
            "rust_reference",
    ...
    for mechanism in mechanisms:
        connection.execute(
            """
            INSERT INTO mechanisms(mechanism_id, canonical_symbol, mechanism_family, enforcement_kind, stability)
            VALUES(?, ?, ?, ?, ?)
            """,
            (
                mechanism["mechanism_id"],
    ...
    for evidence in mechanism_evidence:
        connection.execute(
            """
            INSERT INTO mechanism_evidence(
                evidence_id, mechanism_id, section_id, statement_id, source_anchor,
                evidence_kind, text_excerpt, confidence, source_fetched_at
            ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                evidence["evidence_id"],
    ...
    for row in table_rows:
        connection.execute(
            "INSERT INTO table1_rows(row_node_id, row_idx, row_marker, table_ref, requirement_text) VALUES(?, ?, ?, ?, ?)",
            (
                row["row_node_id"],
                int(row["row_idx"]),
                row["row_marker"],
        ...
        for footnote_order, footnote_text in enumerate(row.get("row_footnotes", []), start=1):
            connection.execute(
                "INSERT INTO table1_row_footnotes(row_node_id, footnote_order, footnote_text) VALUES(?, ?, ?)",
                (row["row_node_id"], int(footnote_order), str(footnote_text)),
            )
        for term_order, term in enumerate(row.get("row_profile_terms", []), start=1):
            connection.execute(
                "INSERT INTO table1_row_profile_terms(row_node_id, term_order, term, term_source) VALUES(?, ?, ?, ?)",
                (row["row_node_id"], int(term_order), str(term), "curated"),
            )
    for verdict in row_verdicts:
        connection.execute(
            "INSERT INTO row_verdicts(row_node_id, verdict, rationale, rationale_anchor, rationale_timestamp) VALUES(?, ?, ?, ?, ?)",
            (
                verdict["row_node_id"],
                verdict["verdict"],
                verdict["rationale"],
    ...
    for row_mechanism in row_mechanisms:
        connection.execute(
            "INSERT INTO row_mechanisms(row_node_id, mechanism_id, relevance_score, evidence_anchor, evidence_section_id, evidence_statement_id, source_fetched_at) VALUES(?, ?, ?, ?, ?, ?, ?)",
            (
                row_mechanism["row_node_id"],
                row_mechanism["mechanism_id"],
                row_mechanism["relevance_score"],
    ...
    for model in semantic_models:
        connection.execute(
            "INSERT INTO semantic_models(model_id, model_role, model_name, model_revision, embedding_dim, distance_metric, license, provider, retrieval_mode, created_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                model["model_id"],
                model["model_role"],
                model["model_name"],
    ...
    for corpus_row in semantic_corpus:
        connection.execute(
            "INSERT INTO semantic_corpus(corpus_id, source_kind, source_id, row_node_id, mechanism_id, source_anchor, text, text_sha256, source_fetched_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                corpus_row["corpus_id"],
                corpus_row["source_kind"],
                corpus_row["source_id"],
    ...
    for score_row in row_mechanism_scores:
        connection.execute(
            "INSERT INTO row_mechanism_scores(row_node_id, mechanism_id, lexical_score, semantic_score, reranker_score, hybrid_score, score_version, top_statement_id, scored_at, source_fetched_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                score_row["row_node_id"],
                score_row["mechanism_id"],
                float(score_row["lexical_score"]),
