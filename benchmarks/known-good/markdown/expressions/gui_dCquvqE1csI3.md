---
guideline_id: gui_dCquvqE1csI3
source_path: src/coding-guidelines/expressions/gui_dCquvqE1csI3.rst
source_sha: 1c7d6bb9448a25401db3b411d1783cd4e78fc718
tier: strict
title: 'Ensure that integer operations do not result in arithmetic overflow'
metadata:
  id: gui_dCquvqE1csI3
  category: required
  status: draft
...
Use explicit `std::num::Wrapping` or
`std::num::Saturating` semantics where these behaviors are intentional.
Range checking can be used to eliminate the possibility of arithmetic overflow.

## Non-Compliant Example 1 (non_compl_ex_cCh2RQUXeH0N)
...
## Non-Compliant Example 2 (non_compl_ex_cCh2RQUXeH0O)
...
## Compliant Example 1 (compl_ex_BgUHiRB4kc4b_1)
...
## Compliant Example 2 (compl_ex_BgUHiRB4kc4c)
...
## Compliant Example 3 (compl_ex_BgUHiRB4kc4b)
...
## Compliant Example 4 (compl_ex_BhUHiRB4kc4b)

Wrapping behavior call also be achieved using the `std::num::Wrapping` type as in this compliant solution.
The `std::num::Wrapping` type is a `struct` found in the `core::num` module that explicitly enables two's complement
wrapping arithmetic for the inner type `T` (which must be an integer or `usize/isize`).
The `std::num::Wrapping` type provides a consistent way to force wrapping behavior in all build modes,
...
## Compliant Example 5 (compl_ex_BgUHiSB4kc4b)
...
## Compliant Example 6 (compl_ex_BgUHiSB4kd4b)

This compliant solution uses the `std::num::Saturating` type to define several functions that perform basic integer operations using saturation semantics.
...
## Compliant Example 7 (compl_ex_BgUHiRB4kc4d)
...
- gui_dCquvqE1csI3:FLS-ARITHMETIC-OVERFLOW :: The Rust FLS. "Expressions - Arithmetic Overflow." https://rust-lang.github.io/fls/expressions.html#arithmetic-overflow

## Citation Signals

- citations: gui_dCquvqE1csI3:FLS-ARITHMETIC-OVERFLOW
- std_refs: std::num::Saturating, std::num::Wrapping, u32::overflowing_add, u32::overflowing_mul, u32::overflowing_sub
